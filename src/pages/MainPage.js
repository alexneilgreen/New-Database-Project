...
const MainPage = () => {
  ...
  const [isCenteredOnIcon, setIsCenteredOnIcon] = useState(false);
  ...
  useLayoutEffect(() => {
    ...
    if (!mapRef.current) {
      ...
      mapRef.current.on("moveend", setIsCenteredOnIcon(false)); // Listen for moveend event to detect map panning or zooming
  ...
  function addIconToMap(long, lat) {
    ...
    if (iconLayer) {
      ...
      console.log("Vector layer not found. Could not place icon");
    }
  }
  ...
  return (
    <div>
      ...
      <div className="main-container">
        <Feed addIconToMap={addIconToMap} centerMapOn={centerMapOn} />
